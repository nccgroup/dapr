...
var libc = null;
const ioctl = new NativeFunction(
  Module.findExportByName(libc, "ioctl"),
  "int",
  ["int", "ulong", "...", "pointer"]
);
...
function injectIoctl(fd, request, data) {
  var _request = typeof request === "string" ? parseInt(request, 16) : request;
  var _data;
  ...
  var outData = null;
  if (!!data) {
    var outData = Array.prototype.slice.call(
      new Uint8Array(Memory.readByteArray(_data, data.length))
    );
  }
